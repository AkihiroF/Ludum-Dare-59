...
namespace Code
{
    ...
    public class PrefabPlacer : MonoBehaviour
    {
        ...
        [ContextMenu("Clear")]
        public void Clear(bool withTerrain = true)
        {
        ...
        public async void Generate()
        {
            try
            {
                ...
                bool isClearWithTerrain = false;
                foreach (var data in dataCollection)
                {
                    foreach (var info in data.Prefabs)
                    {
                        info.prefab.SetActive(true);
                        if (info.storeToTerrain)
                        {
                            isClearWithTerrain = true;
                            if (targetTerrain == null)
                            {
                                targetTerrain = FindAnyObjectByType<Terrain>();
                            }
                ...
                for (int i = 0; i < infos.Count; i++)
                {
                    ...
                    foreach (var pos in targetPositions)
                    {
                        ...
                        var obj = PrefabUtility.InstantiatePrefab(pInfo.prefab, parent) as GameObject;
                        if(obj == null)
                        {
                            Debug.LogError("Prefab null");
                            return;
                        }
                        obj.transform.position = pos + pInfo.offset;
        ...
        private Dictionary<PrefabInfo, int> GetPrototypeIndexes(Terrain terrain)
        {
            ...
            foreach (var data in dataCollection)
            {
                foreach (var prefabInfo in data.Prefabs)
                {
